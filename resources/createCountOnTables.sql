...
CREATE TABLE User (
    ...
    EMAIL VARCHAR(255) NOT NULL,
    INDEX `EMAIL` (`EMAIL`));
DROP TABLE IF EXISTS RegisteredDevices;
CREATE TABLE RegisteredDevices (
    ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    USER INT NULL,
    DEVICE_IDENTIFIER VARCHAR(64) NOT NULL,
    INDEX `USER` (`USER`),
    INDEX `DEVICE_IDENTIFIER` (`DEVICE_IDENTIFIER`));
...
CREATE TABLE Overseeing (
    ...
    USER INT(8) NOT NULL,
    INDEX `AGENT` (`AGENT`,`USER`,`POLLING_AREA`));
...
CREATE TABLE Count (
    ...
    INDEX `DEVICE_IDENTIFIER` (`DEVICE_IDENTIFIER`),
    INDEX `POLLING_AREA` (`POLLING_AREA`,`BALLOT_BOX`));
